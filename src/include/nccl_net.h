...
typedef struct {
  ...
  uint64_t guid;  // Unique identifier for the NIC chip. Important for
                  // cards with multiple PCI functions (Physical or virtual).  todo: 应该就是网卡id.
  int ptrSupport; // NCCL_PTR_HOST or NCCL_PTR_HOST|NCCL_PTR_CUDA  todo: 网卡, 可以访问的pointer类型? 1. 只能访问cpu, 2. 能直接访问gpu(nv提供了基于ib驱动的情况下).
